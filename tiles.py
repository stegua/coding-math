...
class Tiles(namedtuple('Tiles', ['ts'])):
    def _ipython_display_(self):
        ...
        r = Rectangle((0,0), 1, 1, fill = False, linestyle = 'None')
        ax.add_patch(r)    
        plt.axis('equal')
        plt.axis('off')
...
# SCRIPTS DI ESEMPIO DA DARE COME ESERCIZIO
def Chess(n): 
    def R(n, t):
        if n <= 0:
            return t
        return quartet(R(n-1,t), R(n-1,t), R(n-1,t), R(n-1,t))
    
    w = parse(TQ, fill=True, faceColor='white', edgeColor='white')
    b = parse(TQ, fill=True, faceColor='black', edgeColor='black')
    return R(n, quartet(w,b,b,w))

def Squares(n):
    def R(n, t):
        if n <= 0:
            return t
        return quartet(R(n-1,t), R(n-1,t), R(n-1,t), R(n-2,t))
    
    r = parse(TQ, fill=True, faceColor='red', edgeColor='red')
    b = parse(TQ, fill=True, faceColor='blue', edgeColor='blue')
    return R(n, quartet(r,b,b,r))

def Irish(n):
    def R(n, t):
        if n <= 0:
            return t
        return quartet(R(n-3,t), R(n-2,t), R(n-1,t), R(n-3,t))
    
    p1 = parse(EDGE, fill=True, edgeColor='black', faceColor='darkgreen')
    p2 = parse(EDGE, fill=True, edgeColor='black', faceColor='blue')
    return R(n, over(over(p2, rot(p1)), over(rot(rot(p2)), flip(p1))))

def Eye(n):
    def R(n, t):
        if n == 0:
            return t
        return R(n-1, over(t, quartet(blank,blank,blank,rot(rot(t)))))
...
def Spiral(n):
    ...
    arc = parse(ARC)
    t = beside(arc, above(rot(rot(rot(arc))), blank), 2, 1)
    return R(n, t)
